...
class Adagrams {
  ...
  // Method that looks at the array of words and
  ...
  static highestScoreFrom(words) {
    ...
    const highestWords = words.filter(
      word => this.scoreWord(word) === maximumScore
    );
    ...
    const minimumLength = Math.min(
      ...highestWords.map(
        word => word.length === 10 ? 0 : word.length
      )
    );
    // Find the words that have the minimum length
    const shortestWords = highestWords.filter(
      word => (word.length === 10 ? 0 : word.length) === minimumLength
    );
